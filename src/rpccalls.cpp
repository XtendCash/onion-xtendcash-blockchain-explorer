...
bool
rpccalls::connect_to_xtend_daemon()
{
...
rpccalls::get_current_height()
{
    ...
    if (!r)
    {
        cerr << "Error connecting to xtend daemon at "
             << daemon_url << endl;
        return 0;
    }
...
rpccalls::get_network_info(COMMAND_RPC_GET_INFO::response& response)
{
    ...
    if (r)
    {
        ...
        if (!err.empty())
        {
            cerr << "Error connecting to xtend daemon due to "
                 << err << endl;
            return false;
        }
    }
...
rpccalls::get_block(string const& blk_hash, block& blk, string& error_msg)
{
    ...
    {
        cerr << "get_staking_requirement: error connecting to xtend daemon at "
             << daemon_url << endl;
        return false;
    }
...
rpccalls::get_service_node(COMMAND_RPC_GET_SERVICE_NODES::response &res, const std::vector<std::string> &pubkeys)
{
    ...
    if (!result)
        cerr << "Error connecting to xtend daemon at " << daemon_url << endl;
